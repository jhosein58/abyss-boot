...
pub struct CTarget {
    ...
    extern_syms: HashSet<String>,
    renamed_funcs: HashMap<String, String>,
}
...
impl CTarget {
    ...
    fn get_safe_func_name(&mut self, name: &str) -> String {
        if let Some(renamed) = self.renamed_funcs.get(name) {
        ...
        if self.extern_syms.contains(name) || name == "main" {
            ...
            let new_name = format!("abyss_{}", name);
            self.renamed_funcs
...
impl Target for CTarget {
    ...
    fn start_program(&mut self) {
        ...
        self.extern_syms.clear();
        self.renamed_funcs.clear();
    }
    ...
    fn declare_extern_function(
        ...
        if let Some(ref alias) = external_name {
            if alias != name {
                self.extern_syms.insert(name.to_string());
            }
        }
        ...
        if let Some(real) = external_name {
            if real != name {
                self.write(&format!("#define {} {}", name, real));
                self.set_newline_pending();
            }
        }
    }
    ...
    fn declare_function_proto(
        ...
        let safe_name = self.get_safe_func_name(name);

        self.write(&format!(
            "{};",
            self.func_signature(&safe_name, params, return_type, is_variadic)
        ));
        self.set_newline_pending();
    }
